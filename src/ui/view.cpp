...
void View::addPage(PageIndex pageIndex, std::shared_ptr<Page> page, std::shared_ptr<QAction> action) {
    this->addWidget(page.get());

    connect(action.get(), &QAction::triggered, this, [this, pageIndex, action](bool checked = false) {
...
void View::switchPage(PageIndex pageIndex) {
    ...
    if (_currentPageIndex != PageIndex::Nothing) {
        ...
        std::shared_ptr<QAction> currentPageAction = getPageAction(_currentPageIndex);
