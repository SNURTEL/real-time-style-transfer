...
TEST_CASE("Test tensor to cv2 conversion copy") { 
    ...
    REQUIRE(static_cast<unsigned long>(std::abs(out.data - static_cast<uchar*>(in.data_ptr()))) >= in.numel() * torch::elementSize(torch::typeMetaToScalarType(in.dtype())));
    REQUIRE(static_cast<unsigned long>(std::abs(out.data - static_cast<uchar*>(in.data_ptr()))) >= out.total() * out.elemSize());
}
...
TEST_CASE("Test cv2 to tensor conversion copy") { 
    ...
    REQUIRE(static_cast<unsigned long>(std::abs(in.data - static_cast<uchar*>(out.data_ptr()))) >= out.numel() * torch::elementSize(torch::typeMetaToScalarType(out.dtype())));
    REQUIRE(static_cast<unsigned long>(std::abs(in.data - static_cast<uchar*>(out.data_ptr()))) >= in.total() * in.elemSize());
}